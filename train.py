...
def train():
    ...
    for iteration in range(args.start_iter, cfg['max_iter']):
        ...
        # loc_loss += loss_l.data[0]  # lxx changed it for pytorch0.4
        loc_loss += loss_l.item()
        # conf_loss += loss_c.data[0]  # lxx changed it for pytorch0.4
        conf_loss += loss_c.item()

        if iteration % 10 == 0:
            # print('timer: %.4f sec.' % (t1 - t0))
            # print('iter ' + repr(iteration) + ' || Loss: %.4f ||' % (loss.data[0]), end=' ')  # lxx changed it for pytorch0.4
        ...
        if args.visdom:
            # update_vis_plot(iteration, loss_l.data[0], loss_c.data[0], iter_plot, epoch_plot, 'append')  # lxx changed it for pytorch0.4
